...
public final class HuffmanDecoder {
	 ...
	 * @param  bytes the string literal to be decoded
	 * @return the output stream for the compressed data
	 * @throws IOException if an I/O error occurs. In particular,
	 *         an <code>IOException</code> may be thrown if the
	 *         output stream has been closed.
	 */
	public void decode(byte[] bytes, int offset, int length, OutputStream baos) throws IOException {
		...
		for (int i = offset; i < offset+length; i++) {
			...
			while (bits >= 8) {
				...
				bits -= node.bits;
				if (node.isTerminal()) {
					if (node.symbol == Huffman.HUFFMAN_EOS) {
						throw EOS_DECODED;
					}
