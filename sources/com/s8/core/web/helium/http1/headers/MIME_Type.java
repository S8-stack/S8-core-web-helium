package com.s8.core.web.helium.http1.headers;
...
public enum MIME_Type {
	...
	private static Map<String, MIME_Type> mapByKeywords;
	
	static {
		mapByKeywords = new HashMap<String, MIME_Type>(MIME_Type.values().length);
		for(MIME_Type type : MIME_Type.values()){
			mapByKeywords.put(type.keyword, type);
		}
	}
	 ...
	 * @return
	 */
	public static MIME_Type getByKeyword(String keyword){
		return mapByKeywords.get(keyword);
	}
